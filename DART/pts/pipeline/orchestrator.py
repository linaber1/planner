...
class PTSPipeline:
    ...
    # Dual model generation with text-plan projector.
    def generate_dual(
        self,
        user_prompt: str,
        plan_prompt,
        ...
        answer = self.dual.generate_answer(user_prompt, plan_text=plan, use_plan=use_plan)
        return {
            "plan": plan,
            "refined_plan": answer["metadata"].get("refined_plan", ""),
            "answer": answer['text'],
            "metadata": {
                **diff_metadata,
                **answer['metadata']
